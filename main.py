...
def chart():
    ...
    result = client['NBA2']['df3'].aggregate([
        {'$project':{'SEASON':1}},
        {'$project':{'_id':0}}
    ])
    ss = pd.DataFrame(list(result))
    ss = ss.drop_duplicates()
    ss = {'season':ss.SEASON.to_list()}
    
    result = client['NBAgamesData']['teams'].aggregate([
        {'$project':{'NICKNAME':1,'TEAM_ID':1}},
        {'$project':{'_id':0}}
    ])
    tf = pd.DataFrame(list(result))
    cols = tf.columns.to_list()
    cols = cols[-1:] + cols[:-1]
    tf = tf[cols]
    tf = tf.to_dict('split')
    tf = dict(tf['data'])
